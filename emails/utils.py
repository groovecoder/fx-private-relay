...
from privaterelay.sp3_plans import get_sp3_country_language_mapping
...
def get_email_domain_from_settings() -> str:
    return str(urlparse(settings.SITE_ORIGIN).netloc)
...
def get_welcome_email(user: User, format: str) -> str:
    ...
    bundle_plans = get_countries_info_from_lang_and_mapping(
        sa.extra_data.get("locale", "en"), get_sp3_country_language_mapping("bundle")
    )
