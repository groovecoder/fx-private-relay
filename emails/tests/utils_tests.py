...
def test_remove_trackers_does_not_backtrack_on_dotted_url() -> None:
    ...
    An earlier pattern used `(\\S*\\.)*`, whose ambiguous partitioning backtracks
    exponentially at roughly 4x per added dot. One 130-byte link pinned an email
    worker for its whole 120s timeout, about 1200 times a day. See MPP-4739.
    """
